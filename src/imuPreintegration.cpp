...
/*
 ...
 一， TransformFusion类
 功能介绍： 主要订阅激光里程计，根据前一时刻激光里程计和该时刻到当前时刻的
 IMU里程计变换增量，计算当前时刻IMU里程计，rviz展示IMU里程计轨迹。
 ...
 二 ImuPreintegration类
 功能介绍：
 1. 用激光里程计，两帧激光里程计之间的IMU预积分量构建因子图，优化当前帧的
 状态（包括位姿，速度，偏置）
 2. 优化后的状态为基础，施加IMU预计分量，得到每一时刻的IMU里程计

 订阅
 1. 订阅IMU的原始数据，以因子图优化后的激光里程计为基础，施加两帧之间的IMU
 预计分量，预测每一时刻（IMU频率）的IMU里程计。
 2. 订阅激光里程计（来自MapOptimization），用两帧之间的IMU预积分量构建因
 子图，优化当前帧位姿（这个位姿仅用于更新每一个时刻的IMU里程计，以及下一次
 因子凸优化）
...
public:
    ...
    TransformFusion()
    {
        // 如果lidar坐标系与baselink的坐标系不同（激光系与载体系），需要两个坐标系之间的转换关系
    ...
    /*
     * 订阅imu里程计，来自IMUPreintegration
     * 1. 以最近的一帧激光里程计为基础，计算该时刻与当前时刻间imu里程计增量位姿变换，相乘得到当前时刻里程计位姿
